...
| `externo` | Transversal | Cada `EXT-NNN` con su origen, cada recomendación clasificada con evidencia, y el seguimiento de cada `gap` y `conflicto` en el vocabulario cerrado: un ticket o historia existentes, `pendiente de cascada — motivo`, `pendiente de ADR — motivo` o `sin acción — motivo` |
