...
### La documentación tampoco se acopla a un proyecto ni a un stack (momoy 2.26.1, 2.27.0)
...
- **No citar guardias por su número** (la palabra *Guard*, en singular o plural, seguida de una cifra): cada `AGENTS.md` numera las suyas (`SK-27` lo advierte). Se citan por su nombre o se remite a la regla de momoy correspondiente. Para los propios no-goals de una skill se escribe "Non-Goal N".
- **No usar ejemplos del dominio de un proyecto** ni su layout de carpetas (por ejemplo, las rutas de un monorepo concreto).
- **No imponer tecnologías** (momoy 2.27.0): lenguaje, runtime, gestor de paquetes, framework, base de datos, plataforma de CI o IaC y sus comandos se leen de `docs/00_stack_manifest.md`, el artefacto que genera `SK-04`, y nunca se fijan en momoy. Nombrar una herramienta está permitido como ejemplo marcado ("ej.") o en una enumeración que declara agnosticismo. El check detecta la forma mecánica de esta regla: un comando de gestor de paquetes entre backticks, escrito como instrucción, sin marca de ejemplo ni referencia al manifest antes en la misma línea. El resto (una librería impuesta en prosa, un layout de carpetas) queda para la revisión.
