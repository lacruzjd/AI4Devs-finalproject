...
|---|---|---|---|---|
| R-01 | El contrato de API de §4 del README no coincide con el desplegado | gap | `readme.md:622` documenta `POST /api/v1/catalog/recipes`; la ruta real del contrato es `/api/v1/recipes`. Los ejemplos omiten `storageLocationId` y `fromStorageLocationId`, obligatorios | pendiente de cascada — corregir §4 contra `openapi.yaml`, que es la fuente |
| R-02 | La rotación obligatoria de PIN no ocurre | gap | La regla **sí existe**: `apps/backend/prisma/schema.prisma:87` declara `mustChangePin Boolean @default(true)` y `User.ts:26` la aplica en el dominio. Pero `apps/backend/prisma/seed.ts:134` crea el administrador con un `upsert` **directo por Prisma, saltándose el dominio**, y su rama `update` nunca reasienta el campo: una fila que ya esté en `false` no la sana ningún arranque posterior | pendiente de cascada — el bootstrap debe reasentar la marca, no sólo crearla |
...
| R-05 | Documentar que los tests exigen el cliente ORM generado | gap | `AGENTS.md` sí lo documenta en sus comandos canónicos; `readme.md` no lo menciona en ningún sitio | pendiente de cascada — añadir el paso a los pasos de test del README |
...
| R-08 | Segundo esquema Prisma en la raíz, desactualizado | gap | `prisma/schema.prisma`, 108 líneas, frente a las 347 del real en `apps/backend/prisma/` | pendiente de cascada — borrarlo o documentarlo; hay precedente de confusión por leer el equivocado |
...
| R-10 | Las credenciales de revisión son públicas y no se fuerza el cambio | gap | Mismo mecanismo que R-02: el PIN de bootstrap sigue siendo válido porque la marca de rotación no se reasienta | pendiente de cascada — se resuelve con R-02, no por separado |
| R-11 | `render.yaml` sigue diciendo "NO VERIFICADO contra Render real" | gap | `render.yaml:10`, con el servicio desplegado y respondiendo | pendiente de cascada — la nota describe un estado que dejó de ser cierto |
| R-12 | `pnpm.overrides` se ignora con pnpm 10 o superior; fijar `packageManager` | gap | **La premisa no se sostiene en este repositorio:** `ci.yml` fija `version: 9` en sus dos jobs y ambos Dockerfiles ejecutan `corepack prepare pnpm@9 --activate`, de modo que los overrides **sí se aplican** y las fijaciones de CVE de `TK-134` están vigentes. El agujero real es distinto y más estrecho: sin `packageManager` en `package.json`, un desarrollador local con pnpm 10+ los pierde en silencio | pendiente de cascada — declarar `packageManager`, por el entorno local y no por CI |
