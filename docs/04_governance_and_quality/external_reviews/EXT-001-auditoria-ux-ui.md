...
|---|---|---|---|---|
| R-01 | Diseñarlo como apoyo a decisiones, no como CRUD | gap | La raíz es `InventarioRoute`; hay panel FEFO, no un resumen accionable | sin acción — requiere historia nueva; el humano decidirá con `/momoy-spec` |
| R-02 | Navegación por tareas: Movimientos y Alertas de primer nivel | gap | `apps/frontend/src/app/AppNav.tsx`: 5 entradas; movimientos vive en `/ajustes` tras el permiso `roles:manage` | sin acción — requiere historia nueva: decide quién ve el historial |
| R-03 | Navegación simplificada en móvil | no_verificable | Solo hay `@media` de tema y de movimiento reducido; hace falta probar en dispositivos | sin acción — pendiente de `/momoy-verify-live` en tablet y móvil |
| R-04 | Dashboard como pantalla de entrada con alertas prioritarias | gap | Misma evidencia que R-01 | sin acción — requiere historia nueva; el humano decidirá con `/momoy-spec` |
...
| R-07 | Separar "caducado" de "caduca hoy" en la vista FEFO | gap | `apps/frontend/src/shared/components/urgency.ts`: ambos son `critical`; la raíz agrupa en 3 cubetas | sin acción — requiere historia nueva: afecta a la inocuidad (INV-5), descartar no es consumir |
...
| R-10 | Ordenación por nombre, cantidad o caducidad | gap | `CatalogToolbar.tsx` filtra y busca, no ordena | sin acción — requiere historia nueva; mejora menor de usabilidad |
...
| R-13 | Ficha de producto con historial de movimientos | gap | Hay modales de crear, editar y reponer insumo; no existe una ficha con su historial | sin acción — requiere historia nueva; el humano decidirá con `/momoy-spec` |
| R-14 | Errores junto al campo correspondiente | gap | `CreateInsumoModal.tsx` usa un único `ErrorBanner` arriba | sin acción — requiere historia nueva; mejora menor de formularios |
...
| R-16 | Áreas táctiles de 44 × 44 px | conflicto | `DESIGN.md` exige 48 × 48 px, y `SK-11` lo recoge como NFR de accesibilidad | sin acción — el estándar del proyecto es más estricto; rebajarlo exigiría un ADR |
...
| R-21 | Experiencia adecuada en escritorio, tablet y móvil | no_verificable | Sin `@media` de ancho en `src/styles/`; el layout podría adaptarse por grid | sin acción — pendiente de `/momoy-verify-live` en los tres contextos |
