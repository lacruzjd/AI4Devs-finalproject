...
## Descripción
Añadir a `ConsumeRemanenteUseCase` la validación de vencimiento (INV-5): si `expirationDate` es anterior al momento actual, rechazar con `422` mediante una `RemanenteExpiredException`, sin alterar el estado. El descarte sigue permitido.
...
*   **Then** responde `422` con `title: RemanenteExpiredException` y el remanente no cambia
