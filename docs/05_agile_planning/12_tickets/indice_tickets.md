...
*   **[TK-168: Frontera Transaccional del Consumo y el Descarte](kitchen/backend/TK-168.md)** (Backend) — ⏳ Approved. `US-048`: el consumo escribe hasta tres veces y el descarte dos, hoy sin transacción; una caída a mitad descuenta stock y pierde el registro de la pérdida. Sigue el patrón de las tres fronteras ya existentes. Salda la mitigación 2 declarada incumplida en `TK-160`.
*   **[TK-169: Extraer el Mapeo de Movimientos del Repositorio de Stock](kitchen/backend/TK-169.md)** (Backend) — ⏳ Approved. `US-048` / `EXT-002` R-07: el archivo pasó de 386 a 443 líneas y `TK-168` lo engorda más. Se extrae la traducción entre movimiento de dominio y fila de base de datos.
