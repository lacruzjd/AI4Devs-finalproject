...
| **TK-145** | `security` | **Alto** | **Media** | 2 SP | Remediación técnica del defecto latente de `PM-001`: `CORS_ALLOWED_ORIGINS` se valida como texto libre y un origen sin esquema rompería CORS en silencio. Validarlo como lista de URLs con esquema (Guard 14). | ⏳ Approved |
...
| **TK-145** | N/A (Técnico) | Validar `CORS_ALLOWED_ORIGINS` como Lista de URLs con Esquema (PM-001) | `security` | 2 | Must Have | [security/backend/TK-145.md](security/backend/TK-145.md) |
...
*   **[TK-145: Validar `CORS_ALLOWED_ORIGINS` como Lista de URLs con Esquema](security/backend/TK-145.md)** (Backend) — ⏳ Approved. Remediación técnica (Guard 26) del defecto latente descubierto en el postmortem `PM-001` del primer despliegue en Render.
...
*   **[TK-159: Idempotencia y Momento Real en las Escrituras Encolables](shared/backend/TK-159.md)** (Backend) — ✅ Done. `US-044` / `ADR-009`: clave de idempotencia generada por el cliente con unicidad en base de datos, y momento real de la operación acotado por el servidor contra relojes imposibles. Ambos campos opcionales: la tablet sin cola sigue siendo compatible.
*   **[TK-160: Aceptación de Operaciones Diferidas con Varianza](shared/backend/TK-160.md)** (Backend) — ✅ Done. `US-044` / `ADR-009`: la operación encolada se acepta siempre; si bajaría de cero, la cantidad se acota y la diferencia se registra como varianza con motivo obligatorio del catálogo de `ADR-004`, distinguible de la de conteo físico. Rechazo si el turno ya se concilió.
*   **[TK-159-FE: Shell Instalable y Caché de la Aplicación](shared/frontend/TK-159-FE.md)** (Frontend) — ✅ Done. `US-044` / `ADR-008`: manifiesto, service worker e invalidación del caché atada a la versión del release. Cachea el bundle, nunca datos de inventario.
...
*   **[TK-168: Frontera Transaccional del Consumo y el Descarte](kitchen/backend/TK-168.md)** (Backend) — ✅ Done. `US-048`: el consumo escribe hasta tres veces y el descarte dos, hoy sin transacción; una caída a mitad descuenta stock y pierde el registro de la pérdida. Sigue el patrón de las tres fronteras ya existentes. Salda la mitigación 2 declarada incumplida en `TK-160`.
*   **[TK-169: Extraer el Mapeo de Movimientos del Repositorio de Stock](kitchen/backend/TK-169.md)** (Backend) — ✅ Done. `US-048` / `EXT-002` R-07: el archivo pasó de 386 a 443 líneas y `TK-168` lo engorda más. Se extrae la traducción entre movimiento de dominio y fila de base de datos.

*   **[TK-170-FE: Eliminar el Respaldo de Unidad por Identificador de Semilla](stock/frontend/TK-170-FE.md)** (Frontend) — ⏳ Approved. `US-049`: la pantalla de extracción resuelve la unidad con una tabla de ids de demostración y cae a `KG` para todo lo demás. Es código muerto —la unidad está garantizada por construcción— pero el respaldo taparía en silencio un fallo de mapeo de la API.
