...
*   **[TK-159: Idempotencia y Momento Real en las Escrituras Encolables](shared/backend/TK-159.md)** (Backend) — ⏳ Approved. `US-044` / `ADR-009`: clave de idempotencia generada por el cliente con unicidad en base de datos, y momento real de la operación acotado por el servidor contra relojes imposibles. Ambos campos opcionales: la tablet sin cola sigue siendo compatible.
*   **[TK-160: Aceptación de Operaciones Diferidas con Varianza](shared/backend/TK-160.md)** (Backend) — ⏳ Approved. `US-044` / `ADR-009`: la operación encolada se acepta siempre; si bajaría de cero, la cantidad se acota y la diferencia se registra como varianza con motivo obligatorio del catálogo de `ADR-004`, distinguible de la de conteo físico. Rechazo si el turno ya se concilió.
*   **[TK-159-FE: Shell Instalable y Caché de la Aplicación](shared/frontend/TK-159-FE.md)** (Frontend) — ⏳ Approved. `US-044` / `ADR-008`: manifiesto, service worker e invalidación del caché atada a la versión del release. Cachea el bundle, nunca datos de inventario.
*   **[TK-160-FE: Cola Local de Consumo y Descarte](shared/frontend/TK-160-FE.md)** (Frontend) — ⏳ Approved. `US-044` / `ADR-009`: encolado local, sincronización operación a operación al recuperar la red y estado visible por operación (pendiente, aplicada, con varianza, rechazada).

*   **[TK-161-FE: Tramo de Teléfono en el Shell y el Tablero de Cocina](shared/frontend/TK-161-FE.md)** (Frontend) — ⏳ Approved. `US-045` / `SK-05`: tramo `xs` en el shell y la ruta raíz — franja superior, una ficha por fila, urgentes colapsable y filtro de áreas con desplazamiento propio.
