...
| **Instalabilidad** | Manifiesto de aplicación + Service Worker **escrito a mano** | — | `ADR-008` / `US-044` / `TK-159-FE`. **Decisión humana 2026-09-25:** sin Workbox ni `vite-plugin-pwa` — la necesidad es estrecha (cachear el bundle e invalidarlo por versión) y no justifica dos dependencias más que auditar. `apps/frontend/public/sw.js`, registrado con `/sw.js?v=<versión del paquete>` para que un despliegue nuevo instale un service worker nuevo y borre los cachés anteriores. **Nunca cachea `/api/`**: servir stock viejo como actual es peor que no funcionar |
